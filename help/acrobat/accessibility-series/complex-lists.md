---
title: 복합 목록
description: Acrobat에서 접근성을 위해 복잡한 목록에 태그를 지정하는 방법을 살펴보세요.
feature: Accessibility
role: User
level: Intermediate
topics: Accessibility
jira: KT-18731
source-git-commit: fd6ff74f94bd93fbd40086badf8158d4bc968e5a
workflow-type: tm+mt
source-wordcount: '56'
ht-degree: 0%

---

# 복합 목록

Acrobat에서 액세스 가능성을 위한 복잡한 목록에 태그를 지정하는 방법을 알아봅니다. 복잡한 목록에는 중첩된 목록(글머리 기호 내의 글머리 기호 등)과 동일한 목록(또는 글머리 기호) 내의 여러 단락이 포함됩니다.

>[!NOTE]
>
>이 자습서의 일부 기능은 Acrobat Pro, 프리미엄 및 Studio에서만 사용할 수 있습니다.

>[!VIDEO](https://video.tv.adobe.com/v/3477838?captions=kor&quality=12&learn=on&hidetitle=true)
